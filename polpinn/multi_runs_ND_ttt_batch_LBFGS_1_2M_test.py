...
# ==============================================================================
#           NOUVELLE VERSION DE LA FONCTION D'ENTRAÎNEMENT
#               Utilise la méthode Adam -> L-BFGS (single step)
# ==============================================================================
...
def run_enhanced_case(params_pinns: dict, params: dict, S_f: DataAugmentation, S_j: DataAugmentation, output_path: Path):
    ...
    for it in tqdm(range(epochs_phase1), desc="Phase 1 (Adam)", file=sys.stdout):
        ...
        if it % 10 == 0:
            ...
            if L_list_eval[0] < min_loss_val:
                min_loss_val = L_list_eval[0]
                model_opti = copy.deepcopy(model)
    ...
    def closure():
        ...
        if lbfgs_iter[0] % 100 == 0:
            print(f"  L-BFGS Iter: {lbfgs_iter[0]:04d}, Loss: {L.item():.2e}")
        lbfgs_iter[0] += 1
