...
def run_enhanced_case(params_pinns: dict, params: dict, S_f: DataAugmentation, S_j: DataAugmentation, output_path: Path):
    ...
    if params_pinns.get("var_R", False):
        ...
        print(f"Valeur initiale de R: {R_initial_m*1e9:.2f} nm")
    ...
    def closure():
        ...
        L, L_list = cost_enhanced_full_batch(pinn_system, F_solid, F_liquid, S_f, S_j, X_fick_total, X_data_total, X_grad_total, X_interface_total_current, R_prime_norm, D_solid_norm, D_liquid_norm)
        L.backward()
...
def affichage(path: Path):
    ...
    P_grid_denorm = P_from_G(pinn_system(X_grid), X_grid) * coeff_normal
    P_colormap = P_grid_denorm.detach().numpy().reshape(grid_r.shape)
    np.save(data_dir / "P_total.npy", P_colormap)
    np.save(data_dir / "(r, t)_total.npy", (grid_r.numpy(), grid_t.numpy()))
    ...
    X_grid_solid.requires_grad_(True)
    P_grid_solid_denorm = P_from_G(pinn_system(X_grid_solid), X_grid_solid) * coeff_normal
    P_colormap_solid = P_grid_solid_denorm.detach().numpy().reshape(grid_r_solid.shape)
    np.save(data_dir / "P_solid.npy", P_colormap_solid)
    np.save(data_dir / "(r, t)_solid.npy", (grid_r_solid.numpy(), grid_t_solid.numpy()))
    fig, ax = plt.subplots(figsize=(10, 8))
    contour = ax.contourf(grid_r_solid.numpy() * 1e9, grid_t_solid.numpy(), P_colormap_solid, 50, cmap='jet')
    cbar = fig.colorbar(contour, ax=ax); cbar.set_label('Polarisation P(r,t)')
    ax.set_xlabel('Rayon r (nm)'); ax.set_ylabel('Temps t (s)'); ax.set_title(f"Polarisation (Solide Uniquement)\nR prédit = {R_final_m*1e9:.2f} nm")
