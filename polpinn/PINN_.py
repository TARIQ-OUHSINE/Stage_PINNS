...
def run(
    ...
    t_tensor = torch.tensor(data["t"], dtype=torch.float32).view(-1, 1)
    P_tensor = torch.tensor(data["P_moy"], dtype=torch.float32).view(-1, 1)

    # Données sous forme NumPy
    t_np = np.array(data["t"])
    P_np = np.array(data["P_moy"])

    def S_f(t):
        t_cpu = t.detach().cpu().numpy().flatten()
        return torch.tensor(np.interp(t_cpu, t_np, P_np), dtype=torch.float32).to(t.device).view(-1, 1)

    def S_j(t):
        t_cpu = t.detach().cpu().numpy().flatten()
        return torch.tensor(np.interp(t_cpu, t_np, P_np), dtype=torch.float32).to(t.device).view(-1, 1)

    # Ajouter manuellement un attribut .times pour compatibilité avec hypo()
    S_f.times = data["t"]
    S_j.times = data["t"]
    S_f.list_y = data["P_moy"]
    S_j.list_y = data["P_moy"]
    ...
    for it in tqdm(range(params_pinns["epoch"]), desc="Training process"):
        ...
        if update_progress is not None and it % 1 == 0:
    ...
    from polpinn.tool import data_augmentation  # si pas déjà importé

    # Sauvegarder S_f.pkl et S_j.pkl après le training
    from polpinn.tool import data_augmentation
    data_path = Path(__file__).resolve().parents[2] / "data_1"

    # Ces 3 lignes remplacent les anciens appels avec "S_f", "S_j"
    if entry is not None:
        try:
            S_f = data_augmentation(data_path, output_path, S=entry["Serie_f"], name=params["name"], mono=False)
            S_j = data_augmentation(data_path, output_path, S=entry["Serie_j"], name=params["name"], mono=False)
            S_j_mono = data_augmentation(data_path, output_path, S=entry["Serie_j"], name=params["name"], mono=True)
        except Exception as e:
            print(f"[WARNING] Erreur lors de l'augmentation des données : {e}")
