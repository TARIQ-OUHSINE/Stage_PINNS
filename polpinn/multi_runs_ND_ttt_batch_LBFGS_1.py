...
# NOUVEAU: Importer argparse au début du fichier
import argparse

if __name__ == "__main__":
    # NOUVEAU: Utilisation d'argparse pour rendre le script flexible
    parser = argparse.ArgumentParser(description="Lancer un entraînement PINN pour un cas spécifique.")
    ...
    parser.add_argument('--case_name', type=str, required=True, help="Le nom du cas à traiter (ex: 11_58_40_500).")
    
    args = parser.parse_args()

    # --- CONFIGURATION ---
    # Le nom du cas est maintenant un argument !
    ...
    # --- CHEMINS ---
    # Les chemins sont maintenant passés en arguments
    data_file = Path(args.data_file)
    base_output = Path(args.output_dir)

    # --- CHARGEMENT ET PRÉPARATION ---
    print(f"--- Lancement du cas : {EXP_NAME_TO_RUN} ---")
    with open(data_file, "rb") as f: all_data = pickle.load(f)

    # Vérifier si le cas existe dans les données
    if EXP_NAME_TO_RUN not in all_data:
        print(f"ERREUR: Le cas '{EXP_NAME_TO_RUN}' n'a pas été trouvé dans le fichier de données {data_file}.")
        exit() # Arrête le script proprement

    exp_data = all_data[EXP_NAME_TO_RUN]
    solid_data_key, solvent_data_key = "Cris" + CASE, "Juice" + CASE
    
    # Le dossier de sortie est construit à partir du nom du cas
    ...
    print(f"\n=== FIN DE L'EXPÉRIENCE POUR LE CAS {EXP_NAME_TO_RUN} ===")